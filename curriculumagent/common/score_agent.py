"""Script to score and evaluate the final network36 and compare it to the original scores.
"""
import json
import logging
import os
...
@dataclass
class AgentReport:
    """Report that consists of data gathered from evaluating an network36, mainly used for comparing agents and
...
class ScoreL2RPN2020WithNames(ScoreL2RPN2020):
    ...
    def get(
        ...
        """Get the score of the network36 depending on what has been computed.

        Args:
            agent: The network36 you want to score.
            path_save: The path where you want to store the logs of your network36.
            nb_process: Number of process to use for the evaluation.

        Returns:
            all_scores: List of your network36 per scenarios score.
            ts_survived: List of the step number your network36 successfully managed for each scenario.
        ...
        if self.verbose >= 1:
            print("Starts the evaluation of the network36")
...
def score_agent(
    ...
    """Score the given network36 in the given environment, saving logs in log_dir.

    Args:
        agent: The network36 to score.
        env: The environment to score the network36 on.
        log_path: The directory to save network36 logs in.
        seed: Seed used to score the network36.
        name: The name of the network36.
        nb_episodes: How many episodes/chronics the network36 should be scored. Equals 2 by default.
    ...
    Returns:
        The :class:`AgentReport` describing the network36 performance.
    ...
    # Evaluate trained network36
    if name:
        agent_name = f"{name}_{agent.__class__.__name__}"
    else:
        agent_name = f"{agent.__class__.__name__}"
    print(f"Evaluating network36 with name {agent_name}")

    # Start scoring the network36 and time the execution
    timer = Timer()
    with timer:
        if reinit:
            ...
            print("Delition done! Start re-running network36")
...
def load_or_run(
    ...
    """Load the given report at cache_path or score the network36 if it doesn't exist.

    For easier comparison, this function tries to load cached results of the :func:`score_agent` function.
    However, it does not check whether the result is out of date. So if you updated the network36 to score, you either have
    to delete the corresponding cache file or set the overwrite parameter to True.

    Args:
        agent: The network36 to score.
        env: The environment to score the network36 in.
        output_path: The path to a directory where cached results as well as network36 logs of the scoring will be stored.
        name: The name of the report. The network36 class name will be used if this is not set.
