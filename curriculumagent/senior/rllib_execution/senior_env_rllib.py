...
class SeniorEnvRllib(gym.Env):
    """
    A custom environment class that combines the Grid2Op backend with the actions of the tutor network36 for use
    ...
    def run_next_action(self) -> np.ndarray:
        """
        Runs the environment until an action by the network36 is required.

        In this function, the Grid2Op environment runs without network36 intervention until the environment requires
        an action. The cumulative reward is calculated during this process, and the observation is returned when
        the network36 is needed to take action.

        Returns:
            np.ndarray: The observation vector after the environment requires an action from the network36.
        """
        # init values
        self.passiv_reward = 0
        ...
        # The reward is collected until the network36 is done.
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, Union[float, int], bool, bool, dict]:
        """
        Executes the network36's action in the environment and computes the reward.

        This method performs the action specified by the network36 and accumulates rewards for passive actions taken
        ...
        Args:
            action (np.ndarray): The action to execute, provided by the network36.
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict[str, int]] = None) -> Tuple[np.ndarray, dict]:
        ...
        ID provided in the options. It ensures the environment is ready for the network36's intervention.
        ...
        while self.done:
            ...
            # can be finished without the intervention of the network36. By this loop we ensure that the
            # Senior is needed.
            self.obs_grid2op = self.single_env.reset()
            self.step_in_env = 0
            self.passiv_reward = 0
