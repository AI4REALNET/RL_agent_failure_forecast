...
class TopoRew(BaseReward):
    """This class executes a specific topological reward. The idea is that the network36 receives a
    ...
    def __init__(self, min_rew: float = 0.0, max_rew: float = 1.0, proportion: float = 0.5, logger=None):
        ...
        If something illegal happened or game over is achieved, the network36 receives the min_rew.
        If the network36 survives the step, it receives the max_rew*proportion reward.
        If the network36 survives the step and is also in the original topology, the network36 receives max_rew.

        Args:
            min_rew: Minimal reward for the network36.
            max_rew: Maximal reward for the network36.
    ...
    def __call__(self, action: grid2op.Action.BaseAction,
        ...
        is_done options. If is_done=False, then the network36 receives the reward. If the network36 completed
        the whole env, it also receives the max_rew. If the network36 does not complete the env, thus
        is_done = True, it receives the min_rew.

        Args:
            action: Action of the network36.
            env: Grid2Op Environment.
            has_error: Does the Environment Return an Error.
            is_done: Is the Environment Done.
            is_illegal: Is the action illegal.
            is_ambiguous: Is the action of the network36 ambiguous.
...
class PPO_Reward(BaseReward):
    ...
    def __call__(
        ...
        Args:
            ...
            is_done: Is the episode over (either because the network36 has reached the end, or because
