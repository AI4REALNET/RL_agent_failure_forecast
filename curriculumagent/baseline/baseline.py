...
class CurriculumAgent(BaseAgent):
    """
    This is the Baseline of the Curriculum Agent, which in turn is a refurbished network36 based on the submission of
    ...
    The overall network36 follows the typical baseline format. However, given that the original code consists of four
    different sub-agents, we decided to separate the agents in order to make it easier to understand.
    With the full pipeline, the CurriculumAgent consist of a Teacher,a Tutor, a Junior and a Senior network36 that
    ...
    However, if you just want to use the network36 or retrain the Senior (RL-Model), you can use the train method.
    ...
    def __init__(self,
                 ...
                 name: str,
                 **kwargs):
        """ Init Method of the CurriculumAgent. Note that the network36 has to be loaded after initilization in order to
        work.
    ...
    def act(self, observation: grid2op.Observation.BaseObservation,
        ...
        Returns: Action of the network36

        """
        if self.agent is None:
            warnings.warn("The network36 was not yet loaded, thus we return the do-nothing action. "
    ...
    def load(self, path: Union[str, Path],
             ...
             **kwargs) -> None:
        """This method loads the MyAgent and inits the network36.

        We provide three ways to load the network36:
        ...
        Args:
            ...
            actions_path: path, where to find the action sets. This is required to run the network36. If this is not
    ...
    def save(self, path: Union[str, Path]) -> None:
        """ This method saves the model of the network36. Note that this requires the network36 to
        be initialized
        ...
        """
        if self.agent is None:
            raise ValueError("Model can not be saved, because there is no network36. Please initialize the network36.")
    ...
    def train(self, env: Union[grid2op.Environment.BaseEnv, str] = "l2rpn_case14_sandbox",
        ...
        call ray.init() prior to training this network36!
        ...
        assert self.agent is not None, "The network36 has not been loaded. Please load the network36 first!"
        self.save(save_path)
        ...
        logging.info(f"Training complete. Overwriting network36")
    ...
    def train_full_pipeline(
        ...
        ############################################################################################
        # Stage 5.
        # Saving the network36

        agent_path = save_path / "model"
        logging.info(f"Saving the network36 model and building the network36")
    ...
    def __check_paths(self, path: Union[str, Path],
        ...
        Args:
            path: Path, where the network36 should be saved
    ...
    def create_submission(self, path: Union[str, Path] = "./submission"):
        """ Method that takes a saved model and network36 and creates a full submission folder in the provided path.

        This only covers the "basic" CurriculumAgent!

        Note that you use a scaler, you have to change the make network36 method and add the path. Same holds
