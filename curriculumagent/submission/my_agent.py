""" This file is the advanced network36 constisting of a model that can cover different action spaces, rllib
training, scaling and other additional features.

To submit your own network36, just specify the model and action_space_path and if you want a scaler that
...
try:
    # Import if the network36 is coppied into the submission folder
    ...
    # Import if the network36 is copied into the submission folder
...
class MyAgent(BaseAgent):
    def __init__(
        ...
        """The new advanced network36.

        In contrast to the original network36, this network36 enables the implementation of tuple and triple actions,
        as well as use either a keras model.

        Next to the difference in the models and actions, this network36 also has the ability to transform the
        observations based on a provided scaler and/or filter them accordingly.

        Note:
            If you just want to pass this network36 as submission without the CurriculumAgent, copy the content
        ...
        Args:
            ...
            action_space_path: path, where to find the action sets. This is required to run the network36
            ...
            topo: Booling indicator, whether the network36 should revert to original topology if it is possible
            ...
            run_with_tf: Flag to determine if the network36 should use TensorFlow (True) or PyTorch (False).
        """
        # Initialize a new network36.
    ...
    def act_with_id(
        ...
        """Method of the network36 to act.
    ...
    def act( self, observation: grid2op.Observation.BaseObservation, reward: float, done: bool)\
        ...
        Args:
            ...
            done: Whether the network36 is done
